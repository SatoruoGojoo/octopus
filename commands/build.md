...
2. **run-marker**：把當下 ISO 時間戳寫進目標 repo 的 `.claude/.octopus-arena/.run`（目錄不存在先建立）——守門 hook 只在 marker 有效期間生效。本管線於步驟 5 呈報告前刪除。

**給的是 roadmap**（epic 模式）→ Read roadmap，依相依順序對每筆 change 執行本檔全部步驟（0~6）：每筆各自完整走一輪、各自有驗收 merge 硬停點；**前一筆經 TPM merge 後才啟動下一筆**（起跑時重寫 marker），不 merge 即中止後續。
...
- Read `.openspec.yaml` 的 `octopus.status`：
  - `Draft` → **自動鎖定，不請示**：proposal 內若有未定案的決策卡，取建議選項用 Edit 寫進「已拍板決策」表並標註「auto」；用 Edit 把 `octopus.status: Draft` 改為 `Locked`；追記 `.claude/.octopus-arena/decisions.md`（日期｜change 名｜auto-locked｜取用的建議選項）。**自動鎖定時 browser 驗證一律不執行**（opt-in 沒經 TPM 勾選）。step 模式則停下請 TPM 拍板後才鎖
  - `Locked` → 直接續跑
...
### 4. Browser 驗證（opt-in，有勾才做）
只在「tasks 有標 `browser` 且經 TPM 於拍板停點確認」時執行；入口自動鎖定（auto/直接 build Draft）的一律跳過本步。

**由你（主對話 Core）親自操作瀏覽器**——subagent 用不到瀏覽器工具（Claude Code 限制），不要派 agent 做：
- 逐項執行該 task 標註的操作，截圖存證
- 結果（通過/不符預期＋截圖）整理成「Browser 驗證」段，附在驗收報告之後
- 瀏覽器整合不可用（未啟用 `/chrome` 或擴充未裝）→ **不中斷管線**，報告標紅「Browser 驗證未執行＋原因」
...
## 紅線
- 步驟 0 的閘門不可跳過：`Implemented`、已歸檔與缺 status 的拒絕不可被任何理由說服繞過；自動鎖定必留痕（決策表＋Arena），不得無痕改 status
